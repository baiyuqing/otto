...
Child agents cannot start nested agents. The parent can send a follow-up
message to a queued or running child with the `agent_send` tool; the child
...
- No nested sub-agent delegation; parent-to-child `agent_send` delivery waits
