...
- **Sub-agents:** `[agents]` is TOML-only with no CLI flags or environment variables. Agent definitions are discovered on each runner build (startup, `/new`, `/resume`, `/model`); within a session the catalog is fixed. Existing agent roots are appended to Seatbelt read paths only at process start. `max_parallel` must be between 1 and 16; any other value is a startup error.
