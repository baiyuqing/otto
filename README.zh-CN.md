<p align="center">
  <img src="docs/logo.svg" alt="Kite 标志" width="320">
</p>
...
**Kite 是一个使用 Rust 编写、优先使用本地存储的 Agent。**
给它一项任务，它会循环调用模型、按需使用工具，并在需要时压缩上下文。
支持通过 API key 连接 OpenAI-compatible 接口，或通过 `kite login`
...
- **不必一直盯着：** 持久化会话、本地记忆、可复用 Skills、有边界的子代理、定时器；`kite serve` 还可选把飞书消息送进会话 inbox。
...
也只有它跑验收门禁。Kite 同样可以在 **Linux** 上编译和运行，限制见下方
[安全与限制](#安全与限制)。
...
`~/.local/bin/kite`。请确认 `~/.local/bin` 已在 `PATH` 中。若直接运行
...
在 Shell 中将 API key 导出为 `KITE_API_KEY` 环境变量。
将示例地址和模型名称替换为服务提供方的实际配置；接口必须支持流式 Chat Completions。

```bash
./kite --provider openai-compatible --base-url https://example.invalid/v1 --model YOUR_MODEL_ID
```

API key 从配置中的 `api_key_env` 指定变量读取，回退变量为 `KITE_API_KEY`。
...
- [本地服务：kite serve](docs/user-manual.md#agent-server)，可选[飞书 inbound](docs/user-manual.md#feishu-inbound)
...
**macOS** 是受支持的平台；在 **Linux** 上 Kite 能编译能运行，但没有沙箱：
`auto` 和 `seatbelt` 会失败关闭，`bash` 工具根本不会注册，只有显式传
`--sandbox off` 才能执行命令，且完全不受约束。其余功能（文件工具、会话、
记忆、Skills、子代理、MCP、workflow、`kite serve` 和 Web UI、TUI）两个平台一致。
...
- `crates/kite-core`：provider 契约、wire 编解码、会话编解码、agent 循环和配置，可编译到 `wasm32-unknown-unknown`。
- `crates/kite`：原生二进制，包含 CLI、REPL、TUI、工具、沙箱、记忆、Skills、子代理、inbound 适配器和 `kite serve`。
- `crates/kite-web`：把 `kite-core` 编译为 WebAssembly 供 `ui/` 中的浏览器前端使用，前端与二进制共用同一份实现。
