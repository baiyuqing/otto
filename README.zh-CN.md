...
它会持续执行 SQL 语句（默认：`SELECT 1`），通过本地 `mysql` 命令行客户端发起请求，并实时输出延迟与吞吐统计，帮助你快速评估数据库在并发负载下的基础响应能力。
...
- Go 1.24+
- 可访问的 MySQL（或兼容）实例
- 已安装 `mysql` 客户端并可在 `PATH` 中找到（或通过 `-mysql-bin` 指定路径）
...
- 每次查询都会拉起一个外部 `mysql` 进程；统计结果包含该进程调用开销。
- 密码通过命令行参数（`-p...`）传给 `mysql`；在共享环境下请注意进程可见性与命令历史泄露风险。
- 若 DSN 含查询参数，工具仅使用 `?` 前面的数据库名进行连接选择。
