...
//! meant to serve one turn at a time. `kite-core` has no async mutex on the
...
/// lets `kite-core` build for `wasm32-unknown-unknown`.
pub struct Options {
    pub model: String,
...
/// generator returns the empty string: `kite-core` has no clock and no
