...
/// than imported from `kite::memory` because kite-core cannot depend on the
/// native crate.
...
pub fn resolve_memory(
    ...
    let sqlite_path = if !file.memory.sqlite.path.is_empty() {
        ...
        paths::clean(&format!("{home}/.kite/memory/memory.db"))
    };
...
mod tests {
    ...
    fn resolves_defaults() {
        ...
        assert_eq!(runtime.sqlite_path, "/home/u/.kite/memory/memory.db");
