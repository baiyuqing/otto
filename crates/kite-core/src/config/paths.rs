...
//! resolution, on the forward-slash paths Kite resolves against (Kite runs on
//! macOS only).
//!
//! These duplicate a subset of the byte-exact algorithm in `kite::tool::gopath`
//! rather than reusing it: this crate cannot depend on the native `kite` crate,
