...
//! call): the native `kite::config` layer populates the map from the real
...
pub struct Runtime {
    ...
    /// `KITE_API_KEY` fallback). Empty for the `chatgpt` provider.
    pub api_key: String,
...
/// explicit profile) `session`, then the `KITE_PROVIDER`/`KITE_MODEL`
...
pub fn resolve(
    ...
    if selected_profile.is_empty() {
        selected_profile = env_value(env, "KITE_PROFILE");
    }
    if selected_profile.is_empty() {
    ...
    let env_provider = env_value(env, "KITE_PROVIDER");
    if !env_provider.is_empty() {
        provider = env_provider;
    }
    let env_model = env_value(env, "KITE_MODEL");
...
fn resolve_api_key(
    ...
    if !api_key_env.is_empty() {
        ...
        let fallback = env_value(env, "KITE_API_KEY");
...
/// Re-implements `kite::provider::openaicompat::normalize_base_url` on the
/// `url` crate.
///
/// ponytail: this duplicates (rather than calls)
/// `kite::provider::openaicompat::normalize_base_url`, because kite-core cannot
/// depend on the native `kite` crate that owns it. The tests below pin every
...
mod tests {
    ...
    fn precedence_prefers_explicit_profile_env_model_and_profile_base_url() {
        ...
        let runtime = resolve(
            &file,
            &env_map(&[("KITE_MODEL", "env-model"), ("PROFILE_KEY", "secret")]),
            &SessionDefaults {
    ...
    fn cli_overrides_win_over_environment() {
        ...
        let runtime = resolve(
            &file,
            &env_map(&[
                ("KITE_PROVIDER", "openai-compatible"),
                ("KITE_MODEL", "env-model"),
                ("PROFILE_KEY", "secret"),
            ]),
    ...
    fn environment_profile_wins_before_default_profile() {
        ...
        let runtime = resolve(
            &file,
            &env_map(&[("KITE_PROFILE", "current"), ("CURRENT_KEY", "secret")]),
            &SessionDefaults::default(),
            &Overrides::default(),
        )
        .expect("resolve");
    ...
    fn explicit_profile_overrides_environment_profile() {
        ...
        let runtime = resolve(
            &file,
            &env_map(&[("KITE_PROFILE", "env"), ("EXPLICIT_KEY", "secret")]),
            &SessionDefaults::default(),
            &Overrides {
                profile: "explicit".into(),
                ..Default::default()
    ...
    fn uses_kite_api_key_fallback() {
        ...
        let runtime = resolve(
            &file,
            &env_map(&[("KITE_API_KEY", "fallback-secret")]),
            &SessionDefaults::default(),
            &Overrides {
                profile: "local".into(),
                ..Default::default()
