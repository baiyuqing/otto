...
const DEFAULT_SERVER_SOCKET: &str = "~/.kite/kite.sock";
...
mod tests {
    ...
    fn socket_override_wins_over_file() {
        ...
        file.server.socket = "/file/kite.sock".into();
        let runtime =
            resolve_server(&file, &env("/home"), "/override/kite.sock", "").expect("resolve");
        assert_eq!(
            runtime,
            ServerRuntime {
                socket: "/override/kite.sock".into(),
                listen: String::new()
            }
        );
    }
    ...
    fn file_wins_over_default() {
        ...
        assert_eq!(runtime.socket, "/file/kite.sock");
    }
    ...
    fn default_expands_tilde_to_env_home() {
        ...
        assert_eq!(runtime.socket, "/home/.kite/kite.sock");
    }
    ...
    fn plain_absolute_path_is_cleaned_unchanged() {
        let mut file = File::default();
        file.server.socket = "/x/y/../kite.sock".into();
        let runtime = resolve_server(&file, &env("/home"), "", "").expect("resolve");
        assert_eq!(runtime.socket, "/x/kite.sock");
    }
    ...
    fn listen_override_wins_over_everything() {
        ...
        let runtime = resolve_server(&file, &env("/home"), "/override/kite.sock", "127.0.0.1:2")
            .expect("resolve");
        assert_eq!(
            runtime,
            ServerRuntime {
