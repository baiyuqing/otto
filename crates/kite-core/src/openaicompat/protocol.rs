...
/// One `POST /chat/completions` body. Kite always streams, and always asks for
...
/// Streaming options. Kite sets `include_usage` so the last chunk carries the
/// token counts.
...
/// One choice of a chunk. Kite reads only the first stream of a response, but
