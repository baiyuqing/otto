...
//!
//! The wire half of the ChatGPT Responses provider: request translation
//! ([`protocol`]) and the server-sent-event response assembler ([`stream`]).
//! The HTTP half — the client, its timeouts, its redirect policy, and its
//! credential redaction — is not here. It lives in `kite::provider::chatgpt`,
//! which drives these two modules.
//!
//! There is no overflow classifier here, unlike [`crate::openaicompat`]. The
//! client for this backend reads no error body and classifies no status: a
//! non-2xx response becomes `chatgpt responses HTTP {status}` and nothing else,
