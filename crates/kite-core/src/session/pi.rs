...
/// Provider-reported cost on the wire. Kite never computes these values; they
...
/// The payload of a `custom` entry. Kite writes one of these with custom type
/// `kite.runtime` to record the active provider, model, and profile.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct PiCustom {
...
/// Kite's private extension block, nested under a record's `details.kite`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct PiKiteDetails {
...
struct PiDetails {
    #[serde(default, skip_serializing_if = "Option::is_none")]
    kite: Option<PiKiteDetails>,
}

/// Encodes `details` as the `{"kite":{...}}` wrapper Kite writes.
pub fn encode_pi_kite_details(details: &PiKiteDetails) -> Result<Box<RawValue>, PiError> {
    serde_json::value::to_raw_value(&PiDetails {
        kite: Some(details.clone()),
    })
    .map_err(|error| PiError::other(format!("encode Kite Pi details: {error}")))
}

/// Reads Kite's extension block out of a record's `details`.
///
/// Returns `Ok(None)` when the field is absent, `null`, not an object, or has
/// no `kite` key: another writer's details are not an error. A present but
/// invalid task id is rejected, because Kite wrote it and a bad value would
/// silently reattach a message to the wrong sub-agent.
pub fn decode_pi_kite_details(raw: Option<&RawValue>) -> Result<Option<PiKiteDetails>, PiError> {
    ...
    let Some(kite) = details.kite else {
        return Ok(None);
    };
    if !kite.task_id.is_empty()
        && let Err(error) = (ContextMetadata {
            task_id: kite.task_id.clone(),
        })
        .validate()
    {
