//! WebAssembly bindings over `kite-core` for the browser UI.
//!
//! The exports are the SSE frame reader, the transcript reducer, and the
//! TypeScript declarations for the `kite serve` wire types. `ui/src` imports
...
use kite_core::wire::sse;
use kite_core::wire::transcript::{self, Item};
...
mod tests {
    ...
    fn parse_frames_splits_complete_frames_and_keeps_a_partial_one() {
        ...
        let decoded: kite_core::wire::sse::ParsedFrames =
