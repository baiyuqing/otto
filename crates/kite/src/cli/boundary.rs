...
//!
//! The four `kite login` credential values are captured by the caller and
...
use kite_core::safetext::{MAX_SECRET_BYTES, SecretCollector, canonicalize_utf8};
...
pub fn boundary_secret_values(
    ...
    if !collect.add("KITE_API_KEY") || !collect.add(environment_value(inputs, "KITE_API_KEY")) {
        return (collect.values(), false);
    }
...
mod tests {
    use super::*;
    use kite_core::agent::redactor::REDACTION_MARKER;
    ...
    fn the_api_key_name_and_value_are_both_collected() {
        ...
        assert!(complete);
        for expected in ["KITE_API_KEY", "sk-default", "WORK_KEY", "sk-work"] {
    ...
    fn a_secret_that_collides_with_the_system_prompt_closes_the_boundary() {
        ...
        let prompt = "You are Kite, a concise coding agent.";
        let fixed = FixedText {
