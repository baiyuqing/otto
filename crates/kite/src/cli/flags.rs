//! Command-line parsing for the `kite` binary.
//!
//! ponytail: the flag scanner is hand written rather than delegated to a
//! parsing crate. Kite's command line accepts `-name` and `--name`
...
//! actually written on the command line. Seven of Kite's behaviours
...
use kite_core::config::duration::parse_go_duration;
...
pub fn parse_flags(args: &[String], stdout: &mut dyn Write) -> Result<Parsed, ParseFailure> {
    ...
    if !scan.is_empty() {
        return Err(reject("kite: unexpected positional arguments"));
    }
    ...
    if set.visited("approve") {
        return Err(reject("kite: --approve was renamed to --prompt"));
    }
...
fn validate(options: &CliOptions, ui_visited: bool) -> Result<(), ParseFailure> {
    if options.sandbox_set && !matches!(options.sandbox.as_str(), "auto" | "seatbelt" | "off") {
        return Err(reject("kite: --sandbox must be one of auto, seatbelt, off"));
    }
    if options.continue_last && !options.resume_path.is_empty() {
        return Err(reject(
            "kite: --continue and --resume cannot be used together",
        ));
    }
    if options.no_session && (options.continue_last || !options.resume_path.is_empty()) {
        return Err(reject(
            "kite: --no-session cannot be used with --continue or --resume",
        ));
    }
    if !options.archive_path.is_empty() {
        ...
        if !conflict.is_empty() {
            return Err(reject(&format!(
                "kite: --archive cannot be used with {conflict}"
            )));
        }
    }
    if options.serve {
        ...
        if !conflict.is_empty() {
            return Err(reject(&format!(
                "kite: serve cannot be combined with {conflict}"
            )));
        }
    }
    if options.socket_set && !options.serve {
        return Err(reject("kite: --socket requires the serve subcommand"));
    }
    if options.listen_set && !options.serve {
        return Err(reject("kite: --listen requires the serve subcommand"));
    }
    if options.socket_set && options.listen_set {
        return Err(reject(
            "kite: --socket and --listen cannot be used together",
        ));
    }
    if options.open && !options.serve {
        return Err(reject("kite: --open requires the serve subcommand"));
    }
    if options.open && options.socket_set {
        return Err(reject("kite: --open cannot be used with --socket"));
    }
    if options.shell_time_set && options.shell_timeout.is_zero() {
        return Err(reject("kite: --shell-timeout must be greater than zero"));
    }
    if options.max_output_set && options.max_output_bytes <= 0 {
        return Err(reject("kite: --max-output-bytes must be greater than zero"));
    }
    if !matches!(
        ...
        return Err(reject(
            "kite: --thinking must be one of low, medium, high, xhigh, max",
        ));
    }
    if options.prompt_set && options.prompt.trim().is_empty() {
        return Err(reject("kite: --prompt requires a non-empty value"));
    }
    if options.prompt_set && options.ui == "tui" {
        return Err(reject("kite: --prompt cannot be used with --ui tui"));
    }
    Ok(())
}
...
const USAGE: &str = r"Usage: kite [options]
       kite serve [options] [--socket PATH | --listen HOST:PORT [--open]]
       kite login [--status]   sign in with a ChatGPT subscription
       kite logout             remove stored ChatGPT credentials
       kite memory status|forget <id>
       kite mcp list|add|remove|enable|disable|login|logout ...
       kite workflow run|status|resume|fork|approve|reject|cancel ...
       kite sandbox setup [--config PATH] [--cwd PATH]
...
mod tests {
    ...
    fn the_serve_subcommand_is_stripped_before_flag_scanning() {
        let got = options(&["serve", "--socket", "/tmp/kite.sock"]);
        assert!(got.serve);
        assert!(got.socket_set);
        assert_eq!(got.socket, "/tmp/kite.sock");
    }
    ...
    fn help_prints_usage_and_stops() {
        for flag in ["--help", "-h"] {
            ...
            assert!(text.starts_with("Usage: kite [options]\n"), "{text}");
    ...
    fn the_retired_approve_flag_names_its_replacement() {
        assert_eq!(
            rejection(&["--approve", "x"]),
            "kite: --approve was renamed to --prompt\n"
        );
    }
    ...
    fn scanning_stops_at_the_first_positional_argument() {
        assert_eq!(
            rejection(&["extra"]),
            "kite: unexpected positional arguments\n"
        );
        assert_eq!(
            rejection(&["--", "--model", "x"]),
            "kite: unexpected positional arguments\n"
        );
        assert_eq!(rejection(&["-"]), "kite: unexpected positional arguments\n");
    }
    ...
    fn every_invalid_flag_reports_its_allowed_values() {
        let cases: &[(&[&str], &str)] = &[
            (
                &["--sandbox", "docker"],
                "kite: --sandbox must be one of auto, seatbelt, off\n",
            ),
            (
            ...
            (
                ...
                "kite: --archive cannot be used with --continue\n",
            ),
            (
                &["--archive", "s.jsonl", "--resume", "other.jsonl"],
                "kite: --archive cannot be used with --resume\n",
            ),
            (
                &["--archive", "s.jsonl", "--no-session"],
                "kite: --archive cannot be used with --no-session\n",
            ),
            (
                &["--archive", "s.jsonl", "--prompt", "x"],
                "kite: --archive cannot be used with --prompt\n",
            ),
            (
                &["serve", "--ui", "repl"],
                "kite: serve cannot be combined with --ui\n",
            ),
            (
                &["serve", "--prompt", "x"],
                "kite: serve cannot be combined with --prompt\n",
            ),
            (
                &["serve", "--resume", "anything"],
                "kite: serve cannot be combined with --resume\n",
            ),
            (
                &["serve", "--continue"],
                "kite: serve cannot be combined with --continue\n",
            ),
            (
                &["serve", "--archive", "anything"],
                "kite: serve cannot be combined with --archive\n",
            ),
            (
                &["serve", "--no-session"],
                "kite: serve cannot be combined with --no-session\n",
            ),
            (
                &["--socket", "/tmp/kite.sock"],
                "kite: --socket requires the serve subcommand\n",
            ),
            (
                &["--listen", "127.0.0.1:0"],
                "kite: --listen requires the serve subcommand\n",
            ),
            (
                &[
                    "serve",
            ...
            (&["--open"], "kite: --open requires the serve subcommand\n"),
            (
                &["serve", "--open", "--socket", "/tmp/kite.sock"],
                "kite: --open cannot be used with --socket\n",
            ),
            (
                &["--shell-timeout", "0s"],
                "kite: --shell-timeout must be greater than zero\n",
            ),
            (
                &["--max-output-bytes", "0"],
                "kite: --max-output-bytes must be greater than zero\n",
            ),
            (
                &["--thinking", "banana"],
                "kite: --thinking must be one of low, medium, high, xhigh, max\n",
            ...
            (
                &["--prompt", "   "],
                "kite: --prompt requires a non-empty value\n",
            ),
            (
                &["--prompt", "x", "--ui", "tui"],
                "kite: --prompt cannot be used with --ui tui\n",
            ),
        ];
