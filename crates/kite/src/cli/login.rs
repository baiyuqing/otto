//! `kite login`, `kite logout`, and the REPL's `/login` and `/logout`.
//!
...
use kite_core::safetext::{self, SecretCollector};
...
const REPL_SIGNED_IN: &str = "Signed in to ChatGPT. Restart Kite to use the new credentials.";
...
pub fn capture_auth_credentials(path: &Path) -> CapturedAuth {
    ...
    let credentials = match auth::load(path) {
        ...
        // A missing file is the normal state before `kite login`: nothing is
...
/// `kite login [--status]` and `kite logout`.
...
async fn repl_login_with(
    ...
    match args {
        ...
        "" => {
            ...
            if provider != kite_core::config::PROVIDER_CHATGPT {
                let _ = writeln!(
                    stdout,
