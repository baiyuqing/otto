//! The standalone `kite mcp login|logout <server>` CLI.
//!
...
//! restarting Kite is what picks up a token this command wrote or removed.
...
use kite_core::config::{McpAuth, McpServer, McpTransport, resolve_mcp};
use tokio_util::sync::CancellationToken;

use super::login::{SharedWriter, browser_opener};
use super::run::fail;

const USAGE: &str = "usage: kite mcp login <server> | kite mcp logout <server> | kite mcp list | kite mcp add <server> --transport stdio --command CMD [--arg ARG...] [--env KEY=ENVVAR...] [--cwd DIR] | kite mcp add <server> --transport http --url URL [--header KEY=VALUE...] [--auth none|oauth] [--oauth-client-id ID] [--scope SCOPE...] | kite mcp remove <server> | kite mcp enable <server> | kite mcp disable <server>";

/// Runs `kite mcp ...` and returns its exit code.
pub async fn run(
    args: &[String],
...
fn config_path_for(lookup: &HashMap<String, String>) -> Result<PathBuf, String> {
    ...
    Ok([home.as_str(), ".config", "kite", "config.toml"]
        .iter()
        .collect())
}
...
fn run_add(
    ...
    let _ = writeln!(
        stdout,
        "added MCP server {name}; restart Kite to connect it"
    );
    if uses_oauth {
        let _ = writeln!(
            stdout,
            "run 'kite mcp login {name}' before restarting if this server needs OAuth"
        );
    }
    0
}
...
fn run_remove(
    ...
    let _ = writeln!(
        stdout,
        "removed MCP server {name}; restart Kite to apply changes"
    );
    0
}
...
fn run_enabled(
    ...
    let _ = writeln!(
        stdout,
        "{verb} MCP server {name}; restart Kite to apply changes"
    );
    0
}
...
mod tests {
    ...
    fn config_with_server(home: &Path, name: &str, body: &str) -> HashMap<String, String> {
        let directory = home.join(".config").join("kite");
    ...
    /// Runs `kite mcp ...` and returns its exit code, stdout and stderr.
    ...
    async fn add_stdio_server_creates_config_without_literal_secret() {
        ...
        assert!(stdout.contains("restart Kite"), "{stdout}");

        let path = home.path().join(".config/kite/config.toml");
    ...
    async fn add_http_oauth_server_and_list_it() {
        ...
        .await;
        assert_eq!(code, 0, "stderr = {stderr}");
        assert!(stdout.contains("run 'kite mcp login docs'"), "{stdout}");
        ...
        let text =
            std::fs::read_to_string(home.path().join(".config/kite/config.toml")).expect("config");
