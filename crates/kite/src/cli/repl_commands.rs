...
use chrono::Utc;
use kite_core::config::{McpAuth, McpTransport};
...
async fn mcp_login(
    ...
    match result {
        Ok(()) => {
            let _ = writeln!(
                stdout,
                "Signed in to {name}. Restart Kite to use the new credentials."
            );
            Ok(())
        }
...
mod tests {
    ...
    fn stdio_server(name: &str) -> kite_core::config::McpServerRuntime {
        kite_core::config::McpServerRuntime {
            name: name.to_string(),
            enabled: false,
    ...
    fn http_server(name: &str, auth: McpAuth) -> kite_core::config::McpServerRuntime {
        kite_core::config::McpServerRuntime {
            name: name.to_string(),
            enabled: false,
...
",
    ...
    async fn the_task_list_prints_one_line_per_task() {
        ...
        tasks.record_provider_step(
            &reviewed.id,
            kite_core::model::Usage {
                input_tokens: 10000,
                output_tokens: 2310,
                ..kite_core::model::Usage::default()
            },
            "",
            true,
        );
    ...
    async fn task_detail_prints_the_child_steps_between_the_model_and_the_result() {
        ...
        let history = vec![
            ...
            kite_core::model::Message {
                role: kite_core::model::Role::Assistant,
                blocks: vec![kite_core::model::Block {
                    block_type: kite_core::model::BlockType::ToolCall,
                    tool_name: "read".into(),
                    arguments: Some(
                        serde_json::value::RawValue::from_string(r#"{"path":"main.go"}"#.into())
                            .expect("valid JSON"),
                    ),
                    ..kite_core::model::Block::default()
                }],
                ..kite_core::model::Message::default()
            },
            kite_core::model::Message {
                role: kite_core::model::Role::Tool,
                blocks: vec![kite_core::model::Block {
                    block_type: kite_core::model::BlockType::ToolResult,
            ...
            kite_core::model::Message {
                role: kite_core::model::Role::Assistant,
                blocks: vec![kite_core::model::Block {
                    block_type: kite_core::model::BlockType::Text,
