...
use kite_core::config::resolve::{Overrides, Runtime};
...
use kite_core::session::{CURRENT_VERSION, Header, RuntimeMetadata};
...
impl StartupTrace {
    fn from_lookup(lookup: &EnvironmentLookup, started: Instant) -> Self {
        let enabled = matches!(
            lookup.get("KITE_STARTUP_TRACE").map(String::as_str),
...
pub(crate) fn fail(stderr: &mut (dyn Write + Send), message: &str) -> i32 {
    let _ = writeln!(stderr, "kite: {message}");
    1
}

/// Runs one `kite` invocation and returns its exit code.
///
...
pub async fn run(
    ...
    let options = match parse_flags(flag_args, stdout) {
        ...
        Err(ParseFailure::Unsafe) => {
            let _ = stderr.write_all(b"kite: invalid command-line arguments\n");
            return 2;
        }
    ...
    let process_snapshot = environment_snapshot(&host_entries, vec!["KITE_API_KEY".to_string()]);
    ...
    let session_root: PathBuf = [home.as_str(), ".kite", "sessions"].iter().collect();
    ...
    let usage = match crate::usage::Store::open(&Path::new(&home).join(".kite/usage.db")) {
...
impl StartupBoundary {
    ...
    fn redactor(&self) -> kite_core::agent::redactor::Redactor {
...
fn redact_with(redactor: &kite_core::agent::redactor::Redactor, message: &str) -> String {
...
/// The `kite memory` flag grammar names its own config path, so it cannot
/// build a [`CliOptions`].
...
fn load_config(options: &CliOptions, home: &str) -> Result<(String, File), ()> {
    let path: PathBuf = if options.config_path.is_empty() {
        [home, ".config", "kite", "config.toml"].iter().collect()
    } else {
...
/// `KITE_API_KEY`, the selected key name and every configured one, deduplicated
/// and sorted.
pub(super) fn sandbox_provider_environment_names(file: &File, selected: &str) -> Vec<String> {
    let mut names: BTreeSet<String> = BTreeSet::new();
    names.insert("KITE_API_KEY".to_string());
...
/// at, and the default roots include the workspace-relative `.kite/skills`,
...
mod tests {
    ...
    fn the_config_environment_always_holds_every_fixed_key() {
        let mut file = kite_core::config::File::default();
    ...
    fn sandbox_settings_pick_up_existing_skill_and_agent_roots() {
        ...
        std::fs::create_dir(directory.path().join(".kite")).expect("kite dir");
        std::fs::create_dir(directory.path().join(".kite/skills")).expect("skills dir");
        let file = kite_core::config::File::default();
        let settings =
    ...
    /// that clones or writes there controls `.kite/skills`. A symlinked root
    ...
    fn a_symlinked_skill_root_is_not_promoted_to_a_sandbox_read_path() {
        ...
        std::fs::create_dir(directory.path().join(".kite")).expect("kite dir");
        std::os::unix::fs::symlink(&secrets, directory.path().join(".kite/skills"))
            .expect("symlink");
    ...
    async fn startup_trace_environment_writes_timing_lines() {
        ...
        std::fs::create_dir_all(home.join(".config/kite")).expect("config dir");
        std::fs::create_dir_all(&workspace).expect("workspace dir");
        std::fs::write(
            home.join(".config/kite/config.toml"),
            r#"
...
"#,
    ...
    async fn an_unsafe_command_line_exits_two() {
        ...
        assert_eq!(
            ...
            "kite: invalid command-line arguments\n"
        );
    }
    ...
    async fn help_writes_usage_and_exits_zero() {
        ...
        .await;
        assert_eq!(code, 0);
        assert!(String::from_utf8_lossy(&stdout).starts_with("Usage: kite [options]"));
