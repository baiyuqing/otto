...
use chrono::Utc;
use kite_core::agent::redactor::Redactor;
use kite_core::agent::{
    Agent, AgentError, CompactionResult, CompactionSettings, EventSink, Options,
};
use kite_core::config::resolve::{Overrides, Runtime, SessionDefaults};
use kite_core::config::{ConfigError, File, McpRuntime};
use kite_core::model::{Block, Message, ToolDefinition};
use kite_core::provider::{Provider, ProviderError, Request, RequestSizer, Response, StreamSink};
...
use kite_core::tool::ToolExecutor;
...
/// selects; `kite_core::session::MemorySession` carries the transcript and this
...
impl Runner {
    ...
    fn collecting<'a>(
        &'a self,
        emit: EventSink<'a>,
    ) -> impl FnMut(kite_core::agent::Event) + Send + use<'a> {
        move |event| {
            ...
            agent: Agent::new(
                ...
                Options {
                    ...
                    tasks: Some(Arc::clone(&tasks)
                        as Arc<dyn kite_core::agent::tasks::TaskRegistry + Send + Sync>),
                    ..Options::default()
                },
            ),
...
/// `KITE_TRACE` writer. None of them changes the order below; each only appends
...
pub struct Builder {
    ...
    /// The captured `~/.kite/auth/chatgpt.json`.
...
impl Builder {
    ...
    async fn build_runner_inner(
        ...
        } else if runtime.provider == kite_core::config::PROVIDER_CHATGPT {
        ...
        let options = Options {
            ...
            tasks: subagents
                .tasks
                .clone()
                .map(|tasks| tasks as Arc<dyn kite_core::agent::tasks::TaskRegistry + Send + Sync>),
    ...
    pub fn resolve_profile(&self, profile: &str) -> Result<Runtime, BuildError> {
        ...
        kite_core::config::resolve::resolve(
            &self.config,
            &resume_environment(&self.environment),
    ...
    /// starting Kite and quitting without a prompt leaves nothing behind.
...
pub fn resume_environment(environment: &HashMap<String, String>) -> HashMap<String, String> {
    environment
        ...
        .filter(|(key, _)| {
            !matches!(
                key.as_str(),
                "KITE_PROVIDER" | "KITE_PROFILE" | "KITE_MODEL" | "KITE_UI"
            )
        })
...
pub fn resolve_initial_runtime(
    ...
    let Some(metadata) = metadata else {
        return kite_core::config::resolve::resolve(
            file,
            environment,
            &SessionDefaults::default(),
            overrides,
...
mod tests {
    ...
    async fn build_runner_without_mcp_reports_connecting_and_skips_mcp_tools() {
        ...
        builder.mcp = kite_core::config::McpRuntime {
            enabled: true,
            call_timeout_secs: 5,
            connect_timeout_secs: 1,
            servers: vec![kite_core::config::McpServerRuntime {
                name: "slow".to_string(),
                enabled: true,
                transport: kite_core::config::McpTransport::Stdio {
                    command: "kite-mcp-test-nonexistent-command".to_string(),
                    args: Vec::new(),
                    env: Vec::new(),
    ...
    async fn a_chatgpt_runtime_registers_remind() {
        ...
        runtime.provider = kite_core::config::PROVIDER_CHATGPT.to_string();
        let session = SharedSession::memory(Header::default());
        let runner = builder
            .build_runner(&session, &runtime)
            .await
    ...
    async fn the_system_prompt_carries_the_workspace_context_and_hides_the_api_key() {
        ...
        assert!(prompt.starts_with("You are Kite, a concise coding agent."));
    ...
    fn usage_collector_is_bound_to_the_runtime_and_session() {
        ...
        builder
            ...
            .expect("collector")
            .record(&kite_core::agent::Event::ProviderUsage {
                usage: kite_core::model::Usage {
                    input_tokens: 10,
                    output_tokens: 2,
