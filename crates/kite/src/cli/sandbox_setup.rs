//! `kite sandbox setup`: the interactive sandbox permission editor.
//!
...
//! `[sandbox]` table through [`kite_core::config::update_sandbox`].
//!
...
use kite_core::config::{SandboxNetworkMode, resolve_sandbox, update_sandbox};
...
const USAGE: &str = "usage: kite sandbox setup [--config PATH] [--cwd PATH]";
...
/// Runs `kite sandbox ...` and returns its exit code.
pub async fn run(
    args: &[String],
    ...
    let path: PathBuf = match flags.config_path.is_empty() {
        true => [&home, ".config", "kite", "config.toml"].iter().collect(),
    ...
    let file = match &original {
        None => kite_core::config::File::default(),
    ...
    if github {
        ...
        let Ok(resolved) = canonical_directory(&candidate) else {
            return fail(
                stderr,
                "GitHub configuration directory is missing; run gh auth login outside Kite, then retry (set GH_CONFIG_DIR for a custom location)",
            );
        };
    ...
    let _ = write!(
        stdout,
        "\nConfiguration: {} (applies to future Kite processes using this file)\nDriver: auto (Seatbelt)\nNetwork: {network_mode}\nRead paths: {}\nAllowed environment names: {}\n",
    ...
    let launch = |stdout: &mut (dyn Write + Send)| {
        if github {
            let _ = writeln!(
                stdout,
                "Start Kite with: GH_CONFIG_DIR={} kite --config {} --cwd {}",
                shell_quote(&gh_dir),
    ...
    loop {
        ...
        match answer.as_str() {
            ...
            "save" => {
                ...
                let _ = writeln!(stdout, "Saved. Restart Kite to apply these permissions.");
                launch(stdout);
                return 0;
            }
            _ => {
...
mod tests {
    ...
    async fn setup_rejects_a_bad_command_line() {
        for args in [vec![], vec!["install"], vec!["setup", "--bogus", "x"]] {
            ...
            assert_eq!(String::from_utf8_lossy(&stderr), format!("kite: {USAGE}\n"));
        }
    }
