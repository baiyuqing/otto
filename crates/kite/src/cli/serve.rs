//! `kite serve`: the HTTP composition root.
//!
...
use kite_core::config::ServerRuntime;
use kite_core::config::resolve::Runtime;
use kite_core::config::resolve_feishu;
...
pub async fn run(
    ...
    // SIGTERM is how a long-running `kite serve` is asked to shut down; the
...
fn announce_listen(stdout: &mut (dyn Write + Send), address: &str, token: &str, open: bool) {
    ...
    let _ = writeln!(stdout, "kite serve: {url}");
    if open {
        launch_browser(&url);
    }
}
    ...
    fn open_launches_the_printed_url() {
        with_launch_test(|| {
            ...
            assert_eq!(
                ...
                "kite serve: http://127.0.0.1:8787/?token=tok\n"
            );
            assert_eq!(
                take_launches(),
