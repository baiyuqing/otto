...
use kite_core::config::{File, Profile};
use kite_core::model::{Block, BlockType, Message, Role};
...
pub fn builder(workspace_root: &Path, session_root: &Path) -> Builder {
    Builder {
        ...
        usage: None,
        mcp: kite_core::config::McpRuntime {
            enabled: false,
...
pub fn write_skill(root: &Path, name: &str, description: &str, body: &str) {
    let directory = root.join(".kite").join("skills").join(name);
