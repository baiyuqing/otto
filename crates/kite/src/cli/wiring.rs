...
use kite_core::agent::inbox::Inbox;
use kite_core::agent::redactor::Redactor;
use kite_core::config::agents::AgentsRuntime;
use kite_core::config::memory::MemoryRuntime;
use kite_core::config::{McpAuth, McpTransport, Runtime, resolve_agents, resolve_skills};
use kite_core::model::ToolDefinition;
use kite_core::safetext::dynamic_redaction_marker;
...
struct McpConnectOutcome {
    server: kite_core::config::McpServerRuntime,
...
impl Builder {
    ...
    pub fn build_subagents(
        ...
        let provider: Arc<dyn kite_core::provider::Provider + Send + Sync> = match client {
        ...
        let (runner, warnings) = SubagentRunner::new(RunnerConfig {
            ...
            template: OptionsTemplate {
                ...
                compaction: kite_core::agent::CompactionSettings {
            ...
            parent_session: Some(Arc::new(move || {
                kite_core::session::Session::messages(&session)
            })),
        ...
        for outcome in outcomes {
            ...
            match outcome.result {
                ...
                McpConnectResult::NeedsLogin => {
                    ...
                    let _ = writeln!(
                        warnings,
                        "warning: mcp server {:?} needs login: run 'kite mcp login {}'",
...
/// `crates/kite/tests/mcp_stdio.rs` covers the transport itself with a fake
/// server subprocess.
#[cfg(test)]
mod mcp_tests {
    use super::*;
    use kite_core::config::{McpRuntime, McpServerRuntime};
        ...
        let builder = builder_with_servers(
            ...
            vec![
                stdio_server(
                    "slow",
                    true,
                    "kite-mcp-test-nonexistent-command",
                    directory.path(),
                ),
                stdio_server(
                    "off",
        ...
        let builder = builder_with_servers(
            ...
            vec![
                stdio_server(
                    "first",
                    true,
                    "kite-mcp-test-first-missing",
                    directory.path(),
                ),
                stdio_server(
                    "second",
                    true,
                    "kite-mcp-test-second-missing",
                    directory.path(),
                ),
            ],
        );
...
mod tests {
    ...
    fn the_workspace_scope_uses_a_configured_override() {
        ...
        let scope = workspace_memory_scope(&config, "/work/kite").expect("scope");
...
mod catalog_tests {
    ...
    /// the default `~/.kite/...` roots resolve under the first one.
    struct Fixture {
    ...
    fn a_workspace_skill_overrides_a_user_level_skill() {
        ...
        let workspace = write_definition(
            &fixture.workspace.join(".kite/skills"),
            "SKILL.md",
            "repo-notes",
            "repo-notes",
    ...
    fn an_invalid_skill_warns_and_is_skipped_while_the_valid_one_is_listed() {
        let fixture = fixture();
        let root = fixture.home.join(".kite/skills");
    ...
    fn a_workspace_agent_is_listed_in_the_agents_section() {
        let fixture = fixture();
        write_definition(
            &fixture.workspace.join(".kite/agents"),
            "AGENT.md",
            "reviewer",
            "reviewer",
    ...
    fn sandbox_read_paths_pick_up_only_existing_enabled_roots() {
        for (name, make_directories, disabled, want) in [
            ...
            let fixture = fixture();
            let skills = fixture.workspace.join(".kite/skills");
            let agents = fixture.workspace.join(".kite/agents");
            if make_directories {
