...
pub fn parse(args: &[String]) -> Result<Command, String> {
    let Some(action) = args.first().map(String::as_str) else {
        return Err(
            "kite: workflow requires run, status, resume, fork, approve, reject, or cancel".into(),
        );
    };
    match action {
        "run" => {
            let name = args
                ...
                .cloned()
                .ok_or_else(|| "kite: workflow run requires a name".to_string())?;
            let mut input = String::new();
            let mut index = 2;
            while index < args.len() {
                if args[index] != "--input" || index + 1 >= args.len() || !input.is_empty() {
                    return Err("kite: invalid workflow run arguments".to_string());
                }
        ...
        "status" | "approve" | "reject" | "cancel" => {
            if args.len() != 2 {
                return Err(format!("kite: workflow {action} requires one id"));
            }
            Ok(match action {
        ...
        "resume" => {
            let run_id = args
                .get(1)
                .cloned()
                .ok_or_else(|| "kite: workflow resume requires a run id".to_string())?;
            let retry = match &args[2..] {
                [] => None,
                [flag, step] if flag == "--retry" => Some(step.clone()),
                _ => return Err("kite: invalid workflow resume arguments".to_string()),
            };
        ...
        "fork" => {
            let run_id = args
                .get(1)
                .cloned()
                .ok_or_else(|| "kite: workflow fork requires a run id".to_string())?;
            let after_step = match &args[2..] {
                [flag, step] if flag == "--after-step" => step.clone(),
                _ => return Err("kite: invalid workflow fork arguments".to_string()),
            };
            Ok(Command::Fork { run_id, after_step })
        }
        _ => Err(format!("kite: unknown workflow command {action:?}")),
    }
}
...
pub async fn build_controller(
    ...
    let store = Arc::new(Store::open(
        &Path::new(&builder.home).join(".kite/workflows.db"),
    )?);
    store.recover(&builder.workspace_path)?;
    let agents = kite_core::config::resolve_agents(
        &builder.config,
        &builder.environment,
    ...
    let controller = Controller::new(
        ...
        Path::new(&builder.home).join(".kite/workflow-sessions"),
        RuntimeIdentity {
            profile: runtime.profile.clone(),
            provider: runtime.provider.clone(),
            model: runtime.model.clone(),
...
fn workflow_roots(home: &str, workspace: &str) -> Vec<PathBuf> {
    ...
    if !home.is_empty() {
        roots.push(Path::new(home).join(".kite/workflows"));
    }
    roots.push(Path::new(workspace).join(".kite/workflows"));
    roots
}
...
fn lock_workspace(home: &str, workspace: &str) -> Result<Flock<std::fs::File>, String> {
    ...
    let directory = Path::new(home).join(".kite/workflow-locks");
