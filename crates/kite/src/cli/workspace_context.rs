...
/// is indistinguishable from Kite's own sections in the prompt.
...
/// the file cannot forge Kite's own sections.
///
...
mod tests {
    ...
    async fn writes_the_environment_header_cwd_and_platform_date() {
        ...
        // The header names the host Kite is actually running on, so the
