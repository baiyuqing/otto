//! The slice of Go's `net/url` that Kite's credential detection depends on.
//!
...
//! `ResolveEnvironment` refuse an ordinary proxy setting. Only the parts Kite
...
//! Callers canonicalize with [`kite_core::safetext::canonicalize_utf8`].
//!
...
/// The subset of Go's `url.URL` that Kite reads. Fields Kite never inspects
/// (`RawPath`, `RawFragment`, `ForceQuery`, `OmitHost`) are not stored, because
/// they only affect `URL.String`, which Kite does not call.
