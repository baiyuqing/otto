...
/// Failure modes of `kite mcp login`/`logout` and the discovery chain.
...
/// `<home>/.kite/auth/mcp/<server>.json`, next to the ChatGPT credential file
...
/// One `kite mcp login <server>` invocation's inputs.
...
async fn discover_protected_resource(
    ...
    // a loopback port `kite mcp login` will then trust). Only follow it when
