//! `kite serve`: the HTTP+JSON+SSE frontend.
//!
...
use kite_core::model::{Block, MAX_IMAGE_BYTES, Message, Usage};
use kite_core::session::ListResult;
use kite_core::wire::sse::format_frame;
...
/// `kite serve`'s router plus the session registry and turn lifecycle behind
...
mod tests {
    ...
    use kite_core::model::{Block, BlockType, Message, Role, Usage};
    ...
    use kite_core::session::{CURRENT_VERSION, Header, ListResult};
    use kite_core::wire::sse::{Frame, parse_frames};
    ...
    async fn metrics_expose_each_open_session_context() {
        ...
        for want in [
            ...
            format!("kite_session_context_input_tokens_pending{labels} 0"),
        ] {
        ...
        assert!(!body.contains(&format!("kite_session_context_input_tokens{labels}")));
    }
    ...
    async fn posting_a_turn_streams_server_sent_events() {
        ...
        for want in [
            ...
            r#"kite_provider_tokens_total{kind="input"} 5"#,
            r#"kite_provider_tokens_total{kind="output"} 7"#,
        ] {
            assert!(body.contains(want), "missing {want}:\n{body}");
        }
    }
    ...
    async fn cancelling_a_turn_finishes_it_as_canceled() {
        ...
        assert!(
            harness
                .metrics_body()
                .await
                .contains(r#"kite_turns_total{status="canceled"} 1"#)
        );
    }
    ...
    async fn a_failing_turn_finishes_with_the_error_status() {
        ...
        assert!(
            harness
                .metrics_body()
                .await
                .contains(r#"kite_turns_total{status="error"} 1"#)
        );
    }
    ...
    fn notification() -> kite_core::agent::inbox::Notification {
        kite_core::agent::inbox::Notification {
            task_id: "t1".to_string(),
            kind: Some(kite_core::agent::inbox::NotificationKind::TaskFinished),
            text: "done".to_string(),
            usage: None,
        }
    }
    ...
    async fn notify_open_sessions_fans_out_to_every_open_controller() {
        ...
        let first = harness.create().await;
        let second = harness.create().await;
        let notification = kite_core::agent::inbox::Notification {
            kind: Some(kite_core::agent::inbox::NotificationKind::Message),
            text: "[feishu] hello".to_string(),
            ..kite_core::agent::inbox::Notification::default()
        };
    ...
    async fn a_message_wake_logs_inbox_kind() {
        ...
        tasks
            .notifications()
            .push(kite_core::agent::inbox::Notification {
                kind: Some(kite_core::agent::inbox::NotificationKind::Message),
