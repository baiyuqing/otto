...
use kite_core::agent::events::Event;
use kite_core::model::Usage;
use kite_core::wire::events::{WireEvent, to_wire};
use serde::Serialize;
...
mod tests {
    use super::*;
    use kite_core::agent::events::ApiStatus;
    ...
    fn a_provider_api_call_is_measured_but_never_buffered() {
        ...
        assert!(metrics.render().contains(
            r#"kite_provider_api_requests_total{provider="chatgpt",model="gpt-5",status="ok"} 1"#
        ));
    }
    ...
    fn a_finished_tool_call_is_measured_and_buffered() {
        ...
        assert!(
            metrics
                .render()
                .contains(r#"kite_tool_calls_total{tool="bash",status="ok"} 1"#)
        );
    }
    ...
    fn provider_tokens_are_counted_by_kind() {
        ...
        assert!(body.contains(r#"kite_provider_tokens_total{kind="input"} 5"#));
        assert!(body.contains(r#"kite_provider_tokens_total{kind="output"} 7"#));
    }
