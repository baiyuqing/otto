...
use kite_core::model::{Message, Role, Usage};
use kite_core::session::compaction::{
...
impl Store {
    ...
    /// and the initial `kite.runtime` entry.
    ...
    pub fn update_runtime(&self, runtime: &RuntimeMetadata) -> Result<(), PiError> {
        ...
        entry.custom = Some(PiCustom {
            custom_type: KITE_RUNTIME_CUSTOM_TYPE.to_owned(),
    ...
    fn repair_dangling_tool_calls(&self) -> Result<Vec<Warning>, PiError> {
        use kite_core::model::{Block, BlockType};
...
impl StoreState {
    ...
    fn write_initial_records(&self, file: &mut File) -> Result<(i64, PiEntry), PiError> {
        use kite_core::session::pi::{PI_SESSION_VERSION, PiHeader};
...
/// The Kite-level state a decoded file resolves to.
...
pub(crate) fn validate_pi_header(
    header: &kite_core::session::pi::PiHeader,
