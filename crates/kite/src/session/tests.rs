...
#[test]
fn create_writes_pi_v3_header_and_kite_runtime_entry() {
    ...
    assert_eq!(lines[1]["customType"], "kite.runtime");
...
fn create_rejects_invalid_headers() {
    ...
    let cases: Vec<Case> = vec![
        ...
        (
            "zero timestamp",
            Box::new(|header| header.created_at = kite_core::model::zero_time()),
...
#[test]
fn read_header_derives_kite_runtime_metadata() {
    let temp = TempDir::new();
    let (store, _) = new_store(&temp);
    store
...
fn store_update_runtime_persists_change_and_noops_unchanged() {
    ...
    assert_eq!(lines[2]["customType"], "kite.runtime");
...
fn store_append_compaction_rejects_invalid_checkpoint_without_mutation() {
    ...
    let cases: Vec<Case> = vec![
        ...
        (
            "zero timestamp",
            Box::new(|c| c.created_at = kite_core::model::zero_time()),
        ),
    ];
...
fn store_snapshot_tracks_context_input_across_compaction() {
    let temp = TempDir::new();
    let (store, _) = new_store(&temp);
    assert_eq!(store.snapshot(), kite_core::session::Snapshot::default());
...
fn store_implements_the_core_session_trait() {
    ...
    assert!(matches!(
        error,
        kite_core::session::SessionError::Persist(_)
    ));
    store.close().expect("close");
}
...
#[test]
fn open_rejects_old_kite_v1_without_mutation() {
    let temp = TempDir::new();
...
fn store_round_trips_context_metadata() {
    ...
    store
        .append_message(&Message {
            ...
            display: true,
            context_metadata: Some(kite_core::model::ContextMetadata {
                task_id: "t12".into(),
            }),
...
fn append_rejects_a_record_above_the_entry_cap_before_writing() {
    ...
    message.blocks[0].text = "x".repeat(kite_core::session::MAX_SESSION_ENTRY_BYTES + 1);
...
fn list_stops_inspecting_after_limit_and_ignores_older_poison_candidates() {
    ...
    file.set_len(kite_core::session::MAX_SESSION_FILE_BYTES as u64 + 1)
        .expect("grow");
    drop(file);
    ...
    let path = plant_records(
        ...
        &[
            ...
            concat!(
                ...
                "\"timestamp\":\"2026-08-27T12:00:01Z\",\"customType\":\"kite.runtime\",",
