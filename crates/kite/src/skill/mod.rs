...
//! `~/.kite/skills` and `<workspace>/.kite/skills`; the workspace root wins on
//! a name collision.
//!
...
pub fn roots(home: Option<&Path>, workspace: &Path) -> Vec<PathBuf> {
    ...
    if let Some(home) = home {
        roots.push(home.join(".kite").join("skills"));
    }
    roots.push(workspace.join(".kite").join("skills"));
    roots
}
...
mod tests {
    ...
    fn the_workspace_root_is_searched_after_the_home_root() {
        assert_eq!(
            ...
            [
                PathBuf::from("/home/u/.kite/skills"),
                PathBuf::from("/ws/.kite/skills")
            ]
        );
        assert_eq!(
