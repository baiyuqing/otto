...
//! `~/.kite/agents` and `<workspace>/.kite/agents`; the workspace root wins on
//! a name collision.
//!
...
pub fn roots(home: Option<&Path>, workspace: &Path) -> Vec<PathBuf> {
    ...
    if let Some(home) = home {
        roots.push(home.join(".kite").join("agents"));
    }
    roots.push(workspace.join(".kite").join("agents"));
    roots
}
...
mod tests {
    ...
    fn write_policy_frontmatter_is_loaded() {
        ...
        write_agent(
            root.path(),
            "executor",
            "write_policy: owned_paths\nwrite_paths: crates/kite/**, docs/*.md\n",
            "body\n",
        );
        write_agent(
            root.path(),
        ...
        assert_eq!(executor.write_paths, ["crates/kite/**", "docs/*.md"]);
    ...
    fn roots_put_the_home_directory_before_the_workspace() {
        assert_eq!(
            ...
            vec![
                PathBuf::from("/home/u/.kite/agents"),
                PathBuf::from("/w/.kite/agents"),
            ]
        );
        assert_eq!(
            roots(None, Path::new("/w")),
            vec![PathBuf::from("/w/.kite/agents")]
        );
    }
}
