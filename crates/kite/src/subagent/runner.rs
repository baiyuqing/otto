...
use kite_core::agent::inbox::{Inbox, Notification, NotificationKind};
use kite_core::agent::redactor::Redactor;
use kite_core::agent::{Agent, CompactionSettings, Event, EventSink, Options};
use kite_core::model::{Message, Role, ToolDefinition};
use kite_core::provider::{Provider, ProviderError, Request, RequestSizer, Response, StreamSink};
use kite_core::session::{MemorySession, Session};
use kite_core::tool::{ToolExecutor, ToolResult};
...
const GENERIC_SUBAGENT_INSTRUCTION: &str = "You are running as a sub-agent of Kite. Complete only the delegated task below with the available tools, then reply with a self-contained final report. That final message is returned to the caller as your result; nothing else you write is.";
...
impl Session for SharedTranscript {
    ...
    async fn append(&self, message: Message) -> Result<(), kite_core::session::SessionError> {
        self.0.append(message).await
    }

    fn latest_compaction(&self) -> Option<kite_core::session::CompactionMetadata> {
        self.0.latest_compaction()
    }

    async fn append_compaction(
        &self,
        checkpoint: kite_core::session::CompactionCheckpoint,
    ) -> Result<kite_core::session::CompactionMetadata, kite_core::session::SessionError> {
...
mod tests {
    ...
    use kite_core::agent::inbox::NotificationKind;
    use kite_core::model::{Block, BlockType, FinishReason, Message, Role, Usage};
    use kite_core::provider::Response;
    use std::sync::Mutex;
    ...
    async fn owned_paths_allows_only_matching_mutations() {
        ...
        let tools = ChildTools {
            ...
            write_paths: vec!["crates/kite/**".to_string(), "docs/*.md".to_string()],
        };

        let allowed = tools
            .execute(
                "write",
                &raw(r#"{"path":"crates/kite/src/lib.rs","content":"x"}"#),
                &CancellationToken::new(),
            )
            .await;
        ...
        let denied = tools
            .execute(
                "write",
                &raw(r#"{"path":"crates/kite-core/src/lib.rs","content":"x"}"#),
                &CancellationToken::new(),
            )
            .await;
