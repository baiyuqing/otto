    ...
    definition(
        ...
        serde_json::json!({
            ...
            "properties": {
                ...
                "model": {
                    "type": "string",
                    "description": "Provider model id for the sub-agent. Default: this session's model. Kite does not validate it; an id the endpoint rejects fails the task with the provider's error. Overrides the definition's model."
                },
                "agent": {
                    "type": "string",
    ...
    let texts: Vec<String> = completed
        ...
        .map(|task| {
            tasks.notifications().remove(
                &task.id,
                kite_core::agent::inbox::NotificationKind::TaskFinished,
            );
...
fn history_lines(history: &[kite_core::model::Message]) -> Vec<String> {
    let mut lines = Vec::new();
    for message in history {
        if message.role != kite_core::model::Role::Assistant {
            continue;
        }
        for block in &message.blocks {
            if block.block_type != kite_core::model::BlockType::ToolCall {
                continue;
            }
