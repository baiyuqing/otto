...
use kite_core::safetext::{dynamic_redaction_marker, secret_forms};
...
/// still gets the `PATH` Kite passes it.
#[cfg(target_os = "macos")]
...
pub fn bash_definition_with_approvals() -> ToolDefinition {
    definition(
        "bash",
        "Execute a shell command from the workspace. Set sandbox_permissions to require_escalated only when sandboxed execution cannot complete the task; include a justification. Only the user can grant it, by typing /approve in Kite, and the command does not run until they do.",
        json!({
            "type": "object",
            "additionalProperties": false,
            "properties": {
...
Only the user can approve it, by typing /approve {id} in Kite. That is not a shell \
...
mod tests {
    ...
    async fn elevated_command_requires_an_exact_one_shot_approval() {
        ...
        assert_eq!(
            requested.content,
            "unsandboxed execution was not approved; the command did not run. Only the user can approve it, by typing /approve approval-1 in Kite. That is not a shell command, so do not try to run it yourself. Ask the user for approval and then reissue this exact command; if you continue without it, tell the user what you are doing instead. command=\"git push\"; justification=\"push the reviewed branch\""
        );
        ...
        assert_eq!(
            requested.content,
            "unsandboxed execution was not approved; the command did not run. Only the user can approve it, by typing /approve approval-3 in Kite. That is not a shell command, so do not try to run it yourself. Ask the user for approval and then reissue this exact command; if you continue without it, tell the user what you are doing instead. command=\"git push\"; justification=\"push the reviewed branch\""
        );
        approvals
    ...
    async fn an_exhausted_marker_set_suppresses_all_result_text() {
        let (_dir, workspace) = temp_workspace();
        let marker = kite_core::safetext::dynamic_redaction_marker(&[])
    ...
    async fn it_runs_a_real_command_through_the_direct_driver() {
        ...
        let tool = BashTool::new(
            ...
            strings(&[
                ...
                "ENV=",
                "LC_ALL=C",
                "KITE_DIRECT_VALUE=deterministic",
            ]),
            Duration::from_secs(3),
            4096,
            &[],
    ...
    async fn it_registers_with_an_explicit_empty_environment() {
        ...
        let result = kite_core::tool::ToolExecutor::execute(
            &registry,
            "bash",
            &raw(r#"{"command":"true"}"#),
            &CancellationToken::new(),
