...
use kite_core::session::types::SessionInfo;
...
/// held when Kite started, then every line submitted since, plus the draft a
/// recall interrupted.
///
...
pub(crate) struct App {
    ...
    /// for mouse reporting takes its own drag-selection away, so Kite draws
...
/// final [`kite_core::agent::CompactionResult`] (as opposed to a streamed
...
mod tests {
    ...
    fn a_picker_row_names_the_session_workspace() {
        let row = session_row(&SessionInfo {
            ...
            cwd: "/Users/u/Work/code/kite".into(),
            ..SessionInfo::default()
        });
        assert_eq!(row.label, "review — check the diff [kite]");
        ...
        assert_eq!(current.label, "review (current) —  [kite]");
    ...
    async fn submitting_a_prompt_echoes_it_into_the_transcript() {
        ...
        let mut app = App::new(&controller);
        let cancel = CancellationToken::new();
        app.input = "hello kite".chars().collect();
        app.cursor = app.input.len();
        ...
        assert!(matches!(&action, Some(Action::Prompt(line)) if line == "hello kite"));
        let entry = app.entries.last().expect("entry");
        assert_eq!(entry.kind, Some(EntryKind::User));
        assert_eq!(entry.raw, "hello kite");
    }
}
