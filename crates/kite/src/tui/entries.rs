...
//!
//! It operates on the native `kite_core::model::{Message, Block}` types rather
//! than the JSON-wire transcript in `kite_core::wire`, because that is what
...
use kite_core::session::COMPACTION_CONTEXT_TYPE;
...
/// of usage totals, in the `i64` `kite_core::model::Usage` uses.
...
mod tests {
    use super::*;
    use kite_core::model::ContextMetadata;
