...
mod tests {
    ...
    async fn a_drawn_turn_shows_a_prompt_a_reply_and_a_tool_call_apart() {
        ...
        assert!(screen.contains("❯ hello kite"), "{screen}");
    ...
    async fn normal_layout_leaves_side_margins() {
        let (_workspace, _sessions, mut app) = app_fixture().await;
        app.push_system("hello from kite");
    ...
    /// Confirms Kite's own transcript text — a run of wide (CJK/emoji)
    ...
    /// not re-test ratatui's own wrapping algorithm, only that Kite's text
    /// reaches it intact.
    #[tokio::test]
