//! `kite serve` over a real loopback listener.
//!
...
/// Waits for the `kite serve:` startup line and returns the base URL and the
/// token parsed from it.
async fn await_startup(stdout: &LockedBuffer, stderr: &LockedBuffer) -> (String, String) {
    for _ in 0..400 {
        let text = stdout.text();
        if let Some(line) = text.strip_prefix("kite serve: ")
            && line.ends_with('\n')
        {
            let line = line.trim();
            let rest = line
