...
//! real PTY and spawns the built `kite` binary against the same scripted
//! loopback server the other binary tests use (see
//! `crates/kite/tests/common`), because there is no way to hand an external
//! integration test a fake `kite::app::Controller`.
//!
...
fn the_tui_renders_a_prompt_reply_and_restores_the_terminal_on_exit() {
    ...
    let config_dir = home.path().join(".config/kite");
    ...
    let mut command = Command::new(env!("CARGO_BIN_EXE_kite"));
    command
        .env_clear()
        .env("HOME", home.path())
    ...
    let mut child = command.spawn().expect("spawn the kite binary");
    ...
    // Kite has to run the text selection itself.
    ...
    assert!(status.success(), "kite --ui tui exited with {status:?}");
