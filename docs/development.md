...
requires `crates/kite-core` and `crates/kite-web` to stay buildable for
...
- `crates/kite-core` (wasm-safe library; no filesystem, network, process, or wall-clock API):
...
- `crates/kite` (native binary):
  ...
  - `session`: native JSONL session storage built on `kite_core::session`
  ...
  - `auth`: ChatGPT OAuth sign-in (`kite login`/`kite logout`), credential storage at `~/.kite/auth/chatgpt.json`, and access-token refresh
...
- `crates/kite-web`: the wasm cdylib the browser UI loads; exports `kite-core`'s wire codecs to JavaScript through `wasm-bindgen`
- `ui/`: the TypeScript browser frontend; a client of `crates/kite`'s HTTP API and `crates/kite-web`'s wasm exports only, with no Rust code of its own and no part in `make check-fast`

Keep provider-specific wire structs inside the two provider implementation
modules. Keep file-tool workspace enforcement inside `crates/kite`'s `tool`
module, session persistence append-only, and `bash` delegated through
`crates/kite`'s `sandbox` module; only explicit sandbox `off` may use direct
execution, and it still starts in the selected workspace.

Keep `crates/kite`'s `memory` module behind its neutral contracts. The agent
...
`/memory`/`/remember` in the REPL, and `kite memory status|forget`) are wired
...
functions the REPL uses (`crates/kite/src/cli/repl_commands.rs`).

Keep `crates/kite`'s `skill` module free of imports from other Kite modules
besides `kite-core`. The skill tool's file reads stay confined to the skill
...
Keep `crates/kite`'s `subagent` module behind the runner's construction path:
...
Keep `crates/kite`'s `mcp` module behind `crate::mcp`'s client, transport, and
OAuth types; `crate::tool::mcp` (the model-facing tool adapter) and
`Builder::connect_mcp` in `crates/kite/src/cli/wiring.rs` are the only two
...
`kite mcp login|logout` in `crates/kite/src/cli/mcp.rs` are the only sign-in
surfaces; a completed sign-in still requires restarting Kite to pick up the
new token. A stdio server that exits stays disconnected until Kite
...
secrets exceed the redaction limits in `kite_core::safetext`, is skipped
...
- Use `Message::validate`, `Block::validate`, `ContextMetadata::validate`, and `Usage::validate` in `kite_core::model`. Every type there is an owned value that derives `Clone`; there are no separate deep-copy helpers. Both `kite_core::session::Session` implementations (`MemorySession` in `kite-core` and `Store` in `crates/kite/src/session/store.rs`) enforce neutral validation and tool-call/result sequencing in `append`. Neutral validation permits transient messages without IDs/timestamps; Pi-specific encoding restrictions stay in `kite_core::session`. When adding fields, update ownership tests.
- `kite_core::provider::Response::message` is the single source of finish reason and usage. `Message::usage == None` means unavailable; `Some(Usage::default())` means explicitly reported zero. Preserve explicit presence (`usage_present` on `CompactionResult` and on the task records) through events, task progress, notifications, aggregates, and supported persistence metadata instead of inferring absence from zero counters. Keep legacy Pi normalization in the decoder.
- Keep provider-token persistence in `crates/kite::usage`: collection maps neutral agent events to content-free records, SQLite only appends and aggregates those records, and frontends query through the server API. Never store prompts, response text, tool arguments, or tool output in the usage database.
...
- `kite_core::tool::ToolResult::persisted_content == None` selects `content`; `Some` selects its value, including `Some("")`; `persisted_text` applies that rule. Preserve redaction and the current-turn full-result overlay. Reuse tool definitions and assembly helpers, including `tool::bash::bash_definition` in `crates/kite`; keep conservative preflight and the `Registry::new` validation.
- Tool arguments decode model output rather than a strict client. An optional list argument deserializes with `tool::empty_as_none`, so an empty list reads as an absent key and a model that sends `[]` beside the arguments it is using is still served; `crates/kite/tests/tool_argument_contract.rs` scans the tool sources for one that does not.
...
- Reuse `kite_core::agent::CompactionResult` as the shared compaction payload. Keep HTTP/SSE DTOs in `crates/kite/src/server` separate from internal structs; update `testdata/server/openapi.yaml` alongside wire changes and preserve existing field meanings.

## Lifecycle and frontend contracts

- Construct `app::Controller` through `Controller::new` (one `Builder`, used by `cli::run`) or `Controller::with_builder` (a shared `Arc<Builder>`, used by `server`); the `Builder` in `crates/kite/src/cli/runtime_builder.rs` is the only session/runtime construction path (`create`, `open`, `resolve_profile`). Do not restore placeholder factories or duplicate runtime construction paths.
...
Linux builds and runs Kite without a sandbox, so its gate is `make check-linux`,
...
```bash
make check-fast     # rustfmt --check, clippy -D warnings, focused kite-core tests, git diff --check
make check          # full macOS gate: check-fast, build, all tests, wasm check+test, PTY test, UI test
make build          # build the Web UI, cargo build --release, then copy ./kite
make install        # run make build, then install ./kite to ~/.local/bin/kite
...
make rust-wasm-check # cargo check -p kite-core and -p kite-web for wasm32-unknown-unknown
make rust-wasm-test  # wasm-pack test --node for kite-core and kite-web
make test-tui       # cargo test -p kite --test tui_pty (needs a real PTY)
make check-linux    # the Linux gate: rust-fmt, rust-lint, rust-test, rust-wasm-check, test-tui
```

`check-fast` runs `rustfmt`, `clippy`, and the focused `kite-core` test suite;
...
run it directly with `cargo test -p kite --test tui_pty` on a host that has
...
session transcript and `~/.kite/usage.db`, never writes, and needs a real
...
Go implementation: the Rust build has no `KITE_TRACE` writer (the name is only
...
and the usage chart, and the `kite-web` wasm package built from
`crates/kite-web`. It needs Node 24+ and wasm-pack, and it is exercised by
...
For development, run `kite serve --listen 127.0.0.1:8787` in one terminal and
`cd ui && KITE_URL=http://127.0.0.1:8787 npm run dev` in another, then open the
Vite URL with the `?token=` query from the `kite serve` startup line. Vite
...
`crates/kite/tests/tui_pty.rs`. Live provider tests are opt-in only and
...
- Never add `--api-key`; API keys come only from environment variables, and ChatGPT credentials only from `kite login`.
...
- Do not describe `bash` as always unsandboxed. Kite defaults to macOS Seatbelt; only explicit sandbox `off` is unsandboxed.
...
examples aligned with the actual CLI flags in `crates/kite/src/cli/flags.rs`.
