...
func TestResumeSuccessNotesCanceledTasks(t *testing.T) {
	...
	updated, resultCmd := resuming.Update(runCommandWithin(t, cmd, time.Second))
	got := updated.(Model)
	if text := lastEntryText(t, got); text != "canceled 1 running tasks" {
		t.Fatalf("entry = %q", text)
	}
	if resultCmd != nil {
		t.Fatal("applySessionResumeResult() cmd = non-nil, want nil (Update's taskUpdateMsg case is the sole re-arm point)")
	}
}
...
func TestResumeResultReconcileStaleNotesCanceledTasksAndDoesNotArm(t *testing.T) {
	...
	got := updated.(Model)
	if next != nil {
		t.Fatalf("reconcileCommittedStaleResume() cmd = %v, want nil (Update's taskUpdateMsg case is the sole re-arm point)", next)
	}
	if text := lastEntryText(t, got); text != "canceled 1 running tasks" {
		t.Fatalf("entry = %q", text)
	}
